...
{
    float4x4 World; // 인스턴스별 월드행렬 (column-major)
    // 필요하면 uint Type; 같은 추가 데이터도 여기 넣을 수 있음
};
...
Texture2D gtxtShadowMap : register(t20); // Shadow SRV
...
float CalcShadow(float3 positionW)
{
    ...
    // bias (너무 크면 떠 보이고, 너무 작으면 지글거림)
    float bias = 0.0015f;

    // SampleCmp: 반환 0~1 (빛 받음 정도)
    float lit = gtxtShadowMap.SampleCmpLevelZero(gssShadowCmp, uv, depth - bias);

    // 너무 어두워지는 문제 완화(바닥/캐릭터 색 빠짐 방지)
...
float4 PSTerrain(VS_TERRAIN_OUTPUT input) : SV_TARGET
{
    ...
    float3 normalW = normalize(cross(dx, dy)); // cross 순서 변경
    if (normalW.y < 0.0f)
        normalW = -normalW; // 위쪽(+Y) 기준으로 뒤집힘 방지(지형에 특히 잘 먹음)

    // 2) 조명
    float3 litRGB = max(Lighting(input.worldPos, normalW).rgb, 0.0f); // 음수 방지

    // 3) 그림자 (diffuse에만 적용하는 쪽이 자연스러움)
    float shadow = CalcShadow(input.worldPos);

    // 4) 합성 (곱셈으로 '전체를 깎아먹지' 않도록)
    //    최소 밝기(ambient floor)를 보장
    float3 ambientFloor = 0.20f; // 여기 0.15~0.3 사이로 취향 조절
...
float4 PS_GS_Billboard(GS_PS_BILLBOARD_OUTPUT input) : SV_TARGET
{
    ...
    float fUOffset = uvParams.x; // _11
    float fVOffset = uvParams.y; // _12
    float fUScale = uvParams.z; // _13
    float fVScale = uvParams.w; // _14
...
float4 PS_UI_TextureAtlas(VS_SPRITE_TEXTURED_OUTPUT input) : SV_Target
{
    ...
    float fUOffset = gmtxTextureTransforms[0][0][0]; // _11
    float fVOffset = gmtxTextureTransforms[0][1][0]; // _12
    float fUScale = gmtxTextureTransforms[0][2][0]; // _13
    float fVScale = gmtxTextureTransforms[0][3][0]; // _14
...
void CS_MotionBlur(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    [unroll]
        for (int i = 1; i <= TAP; ++i)
        {
            float t = (float) i / (float) TAP; // 0~1
            float w = (1.0f - t); // 가까운 샘플에 가중치 더 줌
