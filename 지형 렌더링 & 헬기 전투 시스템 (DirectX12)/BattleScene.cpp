...
bool CBattleScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		{
		case VK_F8:
			m_nTerrainDebugMode = (m_nTerrainDebugMode + 1) % 2; // OFF->WIRE->TESSCOLOR->OFF
			return true;
		}
		break;
	default:
...
void CBattleScene::AnimateObjects(float fTimeElapsed)
{
	...
	if (m_pLights && m_pPlayer)
	{
		...
		// (1,0,0)???? ?????? ????? ???¥å?, ???? ???/?©£??? ???? ?????? ???? ????? ?????.
		// ?¡¾? Y??(?????) ???????? ???? ??????? ?¡¾?/?????? ?©£??? ???? ????????? ???????? ????.
		m_fSunAngle += fTimeElapsed * 0.1f; // ???????? ????? ??¥å? ????
		XMFLOAT3 xmf3SunDir(
			cosf(m_fSunAngle),
			-0.6f - 0.3f * sinf(m_fSunAngle * 0.5f), // ??? ?????(-Y) ??????, ????? ?? ???
			sinf(m_fSunAngle)
		);
		m_pLights[2].m_xmf3Direction = xmf3SunDir;

		// Directional light(2??) ???? + ?¡À???? ????? focus??
...
void CBattleScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	// cbShadowInfo(b14)?? ?????¬ß????? ???¥å?(PS???? Shadow ???©ª?????)
	pd3dCommandList->SetGraphicsRootConstantBufferView(14, m_pd3dcbShadowInfo->GetGPUVirtualAddress());
	...
	if (m_pMirror && m_pMirrorShader)
	{
		...
		// [????] MakeObliqueProjectionMatrix()?? OpenGL ???(clip z ???? [-1,1])?? ?????? ????????,
		// ?? ????????? ??? D3D LH ????????(clip z ???? [0,1])??? ???? ????? ??¢¥?.
		// ????? ????? ??? ??????¥å??? NDC z?? 1?? ??? ? ?????? ?????(???¡À??? ????? ?????)
		// GPU ?????¥ï??? ???¥å??? ??? ??????????, "??¡Ì ?????? ?? ?????" ?????? ???? ??????.
		// (MakeObliqueProjectionMatrix ????? D3D [0,1] ????? ?¡Æ? ??? ????????? ????? ??? ???? ?? ????.)
