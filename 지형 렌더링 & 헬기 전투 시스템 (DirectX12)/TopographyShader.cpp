...
void CExplosionShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, CScene* pOwnerScene){
	...
	struct QUAD_VERTEX
	{
		XMFLOAT2 vLocalPos; // -0.5~+0.5
		XMFLOAT2 vTex;      // 0~1
	};

	QUAD_VERTEX quadVertices[4] =
	{
		{ XMFLOAT2(-0.5f,  0.5f), XMFLOAT2(0.0f, 0.0f) }, // LT
		{ XMFLOAT2(0.5f,  0.5f), XMFLOAT2(1.0f, 0.0f) }, // RT
		{ XMFLOAT2(0.5f, -0.5f), XMFLOAT2(1.0f, 1.0f) }, // RB
		{ XMFLOAT2(-0.5f, -0.5f), XMFLOAT2(0.0f, 1.0f) }, // LB
	};
	...
	{
		OutputDebugStringA("Explosion.dds �ε� ���� (���ҽ��� nullptr �Դϴ�)\n");
	}
...
void CExplosionShader::AnimateObjects(float fTimeElapsed){
	...
	for (auto& exp : m_Explosions)
	{
		...
		const int totalFrames = exp.nSpriteCols * exp.nSpriteRows; // 7 * 1
