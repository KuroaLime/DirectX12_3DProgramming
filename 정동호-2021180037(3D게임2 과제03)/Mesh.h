...
public:
	...
	BOOL CheckRayIntersectionOOBB(XMVECTOR& xmvPickRayOrigin, XMVECTOR& xmvPickRayDirection, float* pfNearHitDistance);
};
...
public:
	...
	CGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale = XMFLOAT3(1.0f, 1.0f, 1.0f), XMFLOAT4 xmf4Color = XMFLOAT4(1.0f, 1.0f, 0.0f, 0.0f));
	CGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList);
	virtual ~CGridMesh();

	CDiffusedTexturedVertex* CreateGridVertices(int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale = XMFLOAT3(1.0f, 1.0f, 1.0f), XMFLOAT4 xmf4Color = XMFLOAT4(1.0f, 1.0f, 0.0f, 0.0f));
	void CreateVertexAndIndexBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, CDiffusedTexturedVertex* pVertices);
...
public:
	...
	CBillboardPointMesh::CBillboardPointMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight)
		: CMesh(pd3dDevice, pd3dCommandList)
	{
		m_nVertices = 1;
		m_nStride = sizeof(BILLBOARD_VERTEX);
		m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;

		BILLBOARD_VERTEX v;
		v.position = XMFLOAT3(0.0f, 0.0f, 0.0f);
		v.size = XMFLOAT2(fWidth, fHeight);

		m_pd3dVertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, &v, m_nStride * m_nVertices, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
